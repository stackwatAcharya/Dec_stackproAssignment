...
const FooterSectionComp: React.FC = () => {

    return <div>
        <div className="w-full border-t border-white pb-10"></div>
        <div className="grid grid-cols-3 justify-between text-[18px]">
            <div className="flex flex-col gap-2 items-start">
                <Link href={'https://www.monstercat.com/about'} target="_blank"><p>ABOUT MONSTERCAT</p></Link>
                <Link href={'https://www.monstercat.com/contact-us'} target="_blank"><p>CONTACT US</p></Link>
                <Link href={'https://www.monstercat.com/careers'} target="_blank"><p>CAREERS</p></Link>
                <Link href={'https://www.monstercat.com/press'} target="_blank"><p>PRESS</p></Link>
            </div>
            <div className="flex flex-col gap-2 items-start">
                <Link href={'https://www.monstercat.com/terms-of-service'} target="_blank"><p>TERMS OF SERVICE</p></Link>
                <Link href={'https://www.monstercat.com/privacy-policy'} target="_blank"><p>PRIVACY POLICY</p></Link>
            </div>
            <div className="flex flex-col gap-4 items-start">
                <p className="pb-2">MONSTERCAT NEWS</p>
                <p className="italic text-[12px]">Dont miss a thing, stay up to date with the latest news from us.</p>
                <div>
                    <div className="flex items-center justify-between cursor-pointer">
                        <input type="text" placeholder={"Enter Email"} className="placeholder:italic placeholder:bg-[#0a0a0a] h-5 focus:outline-0 focus:ring-0" />
        ...
        <div className="w-full border-b border-white pt-10"></div>
        <div className="py-5 flex justify-between items-center pt-10">
            <p className="italic">2011 - 2024 © Monstercat, All Rights Reserved</p>
            <div className="grid grid-cols-7">
                {icons.map((item: any, index: any) =>
                    Object.keys(item).map((key) => (<>
                        <div key={index + 1} className="col-span-1 flex justify-center items-center">
                            <div>{item[key]}</div>
                        </div>
                    </>))
                )}
